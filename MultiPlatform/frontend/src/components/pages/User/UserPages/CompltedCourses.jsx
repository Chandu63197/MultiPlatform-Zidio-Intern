...
const CompletedCourses = ({
  completedCourses = ["Advanced React & Redux"],
}) => {
  // Filter only completed courses from demoCourses
  const completedCourseDetails = demoCourses.filter((course) =>
    completedCourses.includes(course.title)
  );

  return (
    <section className="completed-courses-container">
      <h2>Completed Courses</h2>
      {completedCourseDetails.length === 0 ? (
        <p className="no-completed-courses">
        ...
        <div className="course-grid">
          {completedCourseDetails.map((course) => (
            <div className="course-card" key={course.title}>
              ...
              {/* You can add instructor info here if you want */}

              <button
                className="resume-btn"
                onClick={() => alert(`Reviewing ${course.title}...`)}
              >
                Review Course
              </button>
            </div>
